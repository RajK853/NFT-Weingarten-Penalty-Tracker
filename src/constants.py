...
class Scoring:
    ...
    KEEPER_OUT: float = 0.0 # Score awarded to a goalkeeper for a shot going out (not a save, not a goal).
    PERFORMANCE_HALF_LIFE_DAYS: int = 45 # Days until a performance is worth half its value for time-decay calculations.
...
class UI:
    ...
    MAX_PLAYER_SELECTIONS: int = 10 # Maximum number of players that can be selected in multi-select widgets.
    SCATTER_POINT_SIZE: int = 4 # Default size for scatter plot points.
    MAX_NAMES_IN_METRIC_DISPLAY: int = 3 # Maximum number of names to display in a metric widget before truncating.
    ...
    Y_AXIS_RANGE_MAX: int = 100 # Maximum value for Y-axis in certain charts (e.g., percentages).
    ...
    GOLD_TEXT_STYLE: str = "color: gold; text-shadow: 0 0 2px gold, 0 0 4px gold, 0 0 6px gold;" # CSS style for gold-colored text.
    TOP_N_PLAYERS_LEADERBOARD: int = 10 # Number of top players to display in leaderboards.
    DEFAULT_NUM_PLAYERS_MULTISELECT: int = 4 # Default number of players pre-selected in multi-select.
    ...
    PLOTLY_TEXT_POSITION_OUTSIDE: str = 'outside' # Text position for Plotly annotations.
    PLOTLY_SCATTER_MARKER_SIZE: int = 10 # Marker size for Plotly scatter plots.
    PLOTLY_SCATTER_MARKER_OPACITY: float = 0.7 # Marker opacity for Plotly scatter plots.
    ...
    PLOTLY_BG_COLOR_TRANSPARENT: str = 'rgba(0,0,0,0)' # Transparent background color for Plotly.
    ...
    INFO_SELECT_DATE_RANGE: str = "Please select both a start and end date for the leaderboard." # Info message for date range selection.
    INFO_NO_PLAYER_DATA: str = "No data available for the selected players in {selected_month_display}. Please select different players or a different month. 😔" # Info message when no player data is found.
    INFO_NO_KEEPER_DATA: str = "No data available for {keeper} in {selected_month_display}. 😔" # Info message when no keeper data is found.
    INFO_SELECT_MONTH_KEEPER: str = "Please select a month to view goalkeeper performance. 🗓️" # Info message for selecting month for keeper performance.
    ...
    TAB_OUTCOME_DISTRIBUTION: str = "Outcome Distribution" # Tab title for outcome distribution.
...
class Data:
    ...
    """
    PERCENTAGE_MULTIPLIER: int = 100 # Multiplier to convert a fraction to a percentage.
    ...
    PSEUDO_DATA_OUTPUT_PATH: str = "data/pseudo_penalty.csv" # Path for pseudo data output.
