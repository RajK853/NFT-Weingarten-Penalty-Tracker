...
def create_shot_distribution_chart(data: pd.DataFrame) -> go.Figure:
    ...
    fig.add_shape(
        ...
        x0=0,
        y0=0,
        x1=GoalVisual.GOAL_WIDTH,
        y1=GoalVisual.GOAL_HEIGHT,
        line=dict(
            color=GoalVisual.GOAL_POST_COLOR, width=GoalVisual.GOAL_POST_LINE_WIDTH
        ),
    ...
    fig.add_trace(
        go.Scatter(
            ...
            text=data.apply(
                lambda row: f"Shooter: {row[Columns.SHOOTER_NAME]}<br>Outcome: {row[Columns.STATUS]}",
                axis=1,
            ),
            hoverinfo="text",
        )
    ...
    fig.update_layout(
        ...
        xaxis=dict(
            range=[Data.DEFAULT_FILL_VALUE, GoalVisual.GOAL_WIDTH],
            showgrid=UI.PLOTLY_AXIS_SHOWGRID,
            zeroline=UI.PLOTLY_AXIS_ZEROLINE,
            visible=UI.PLOTLY_AXIS_VISIBLE,
        ),
        yaxis=dict(
            range=[Data.DEFAULT_FILL_VALUE, GoalVisual.GOAL_HEIGHT],
            showgrid=UI.PLOTLY_AXIS_SHOWGRID,
            zeroline=UI.PLOTLY_AXIS_ZEROLINE,
            visible=UI.PLOTLY_AXIS_VISIBLE,
        ),
