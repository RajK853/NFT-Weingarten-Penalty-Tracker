...
def generate_pseudo_data(
    ...
    penalties_per_player_per_day: int = 3,
) -> pd.DataFrame:
    """
    ...
    for week_key, days_in_week in dates_by_week.items():
        ...
        num_days_to_select: int = random.randint(
            Data.MIN_DAYS_PER_WEEK, Data.MAX_DAYS_PER_WEEK
        )
        selected_dates.extend(
            random.sample(days_in_week, min(num_days_to_select, len(days_in_week)))
        )
    ...
    for current_date in selected_dates:
        ...
        for shooter in session_shooters:
            for _ in range(penalties_per_player_per_day):
                ...
                status: str = random.choices(
                    statuses, weights=player_probabilities[shooter]["status"], k=1
                )[0]
...
if __name__ == "__main__":
    ...
    pseudo_df: pd.DataFrame = generate_pseudo_data(
        start_date="2023-01-01", end_date="2025-12-31", penalties_per_player_per_day=3
    )
